...
const AddEditPackagingType = ({ getResponseBack }) => {
  ...
  const handleSave = () => {
    ...
    const apiCall = mode === "add" ? createPackagingTypeApi : updatePackagingTypeApi;
    ...
    apiCall(data)
      ...
      .catch((error) => {
        console.error(`Error ${mode === "add" ? "creating" : "updating"} packaging type:`, error);
        enqueueSnackbar(`Error ${mode === "add" ? "creating" : "updating"} packaging type`, {
          variant: "error",
          anchorOrigin: { vertical: "top", horizontal: "right" },
        });
      })
