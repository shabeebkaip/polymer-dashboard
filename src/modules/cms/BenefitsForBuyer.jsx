...
const BenefitsForBuyer = () => {
  ...
  const pageTitle = language === 'ar' ? 'الشروط والأحكام' : 'Benefits For Buyer';

  const fetchData = async () => {
    try {
      const response = await buyerCrud.fetch();
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      const response = await buyerCrud.updateSection(payload);
  ...
  return (
    <div>
      <div className="w-full p-8">
        <h2 className="pt-2 pb-2 text-xl">Benefits For Buyer</h2>
...
export default BenefitsForBuyer;
