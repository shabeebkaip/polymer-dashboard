...
const AddEditChemicalFamily = ({ getResponseBack }) => {
  ...
  const handleSave = () => {
    ...
    const apiCall = mode === "add" ? createChemicalFamilyApi : updateChemicalFamilyApi;
    ...
    apiCall(data)
      ...
      .catch((error) => {
        console.error(`Error ${mode === "add" ? "creating" : "updating"} chemical family:`, error);
        enqueueSnackbar(`Error ${mode === "add" ? "creating" : "updating"} chemical family`, {
          variant: "error",
          anchorOrigin: { vertical: "top", horizontal: "right" },
        });
      })
