...
const AddEditGrade = ({ getResponseBack }) => {
  ...
  const { gradeModal: open, gradeCrud, mode, loader } = useSelector((state) => state.sharedState);
  ...
  const handleSave = () => {
    ...
    const apiCall = mode === "add" ? createGradeApi : updateGradeApi;
    ...
    apiCall(data)
      ...
      .catch((error) => {
        console.error(`Error ${mode === "add" ? "creating" : "updating"} Grade:`, error);
        enqueueSnackbar(`Error ${mode === "add" ? "creating" : "updating"} Grade`, {
          variant: "error",
          anchorOrigin: { vertical: "top", horizontal: "right" },
        });
      })
