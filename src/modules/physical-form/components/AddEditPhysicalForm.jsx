...
const AddEditPhysicalForm = ({ getResponseBack }) => {
  ...
  const handleSave = () => {
    ...
    const apiCall = mode === "add" ? createPhysicalFormApi : updatePhysicalFormApi;
    ...
    apiCall(data)
      ...
      .catch((error) => {
        console.error(`Error ${mode === "add" ? "creating" : "updating"} physical form:`, error);
        enqueueSnackbar(`Error ${mode === "add" ? "creating" : "updating"} physical form`, {
          variant: "error",
          anchorOrigin: { vertical: "top", horizontal: "right" },
        });
      })
